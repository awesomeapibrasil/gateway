...
impl IncrementalCleaner {
        ...
        T: CacheMetricsProvider + ExpiredKeyProvider,
    {
        ...
        debug!("Starting cleanup cycle {} with aggressiveness {:.2}", cycle_id, aggressiveness);
        ...
        while start_time.elapsed() < max_time && keys_checked < max_keys {
            match cache_provider.get_expired_key_sample(1).await {
                Some(expired_keys) if !expired_keys.is_empty() => {
                    for expired_key in expired_keys {
                        if let Some(removed_size) = cache_provider.remove_expired_key(&expired_key).await {
                            keys_cleaned += 1;
            ...
            if keys_checked < min_keys && start_time.elapsed() < max_time {
                ...
                if let Some(sample_keys) = cache_provider.get_random_key_sample(min_keys - keys_checked).await {
                    for key in sample_keys {
                        if cache_provider.is_key_expired(&key).await {
                            if let Some(removed_size) = cache_provider.remove_expired_key(&key).await {
                                keys_cleaned += 1;
                                memory_freed_bytes += removed_size;
                            }
                            expired_keys_found += 1;
        ...
        T: CacheMetricsProvider,
    {
        ...
        let cache_utilization = cache_provider.cache_size() as f64 / cache_provider.max_size() as f64;
    ...
    fn update_stats(&self, keys_cleaned: usize, time_taken: Duration) {
        ...
        self.stats.total_keys_cleaned.fetch_add(keys_cleaned as u64, Ordering::Relaxed);
        self.stats.total_cleanup_time_ms.fetch_add(time_ms, Ordering::Relaxed);
        self.stats.last_cycle_keys_cleaned.store(keys_cleaned, Ordering::Relaxed);
        self.stats.last_cycle_time_ms.store(time_ms, Ordering::Relaxed);
        ...
        if total_cycles > 0 {
            self.stats.avg_cleanup_time_ms.store(total_time / total_cycles, Ordering::Relaxed);
        }
        ...
        T: CacheMetricsProvider + ExpiredKeyProvider + Send + Sync + 'static,
    {
        tokio::spawn(async move {
            ...
            info!("Starting background cleanup task with interval {:?}", self.config.cleanup_interval);
                ...
                match self.run_cleanup_cycle(cache_provider.as_ref()).await {
                    result => {
                        if result.keys_cleaned > 0 {
                            debug!("Background cleanup: removed {} keys in {:?}", 
...
impl AdaptiveController {
    ...
    fn recommended_aggressiveness(&self) -> f64 {
        ...
        let avg_memory_usage: f64 = recent_metrics.iter()
            .map(|m| m.memory_usage)
            .sum::<f64>() / recent_count as f64;
            
        let avg_hit_rate: f64 = recent_metrics.iter()
            .map(|m| m.hit_rate)
            .sum::<f64>() / recent_count as f64;
            
        let avg_expired_ratio: f64 = recent_metrics.iter()
            .map(|m| m.expired_ratio)
            .sum::<f64>() / recent_count as f64;
...
mod tests {
    ...
    impl CacheMetricsProvider for MockCacheProvider {
        fn memory_usage_ratio(&self) -> f64 { self.memory_usage }
        fn hit_rate(&self) -> f64 { self.hit_rate }
        fn cache_size(&self) -> usize { self.data.lock().unwrap().len() }
        fn max_size(&self) -> usize { self.max_size }
        fn estimated_expired_ratio(&self) -> f64 { 0.1 }
    }
