...
use super::{UDPMulticastCluster, ClusterMessage, MessageType, ClusterPayload, CacheOp};
...
impl CacheCoherenceManager {
    ...
    pub async fn start(&mut self) -> Result<(), CoherenceError> {
        ...
        info!("Cache coherence manager started with strategy: {:?}", self.config.strategy);
        Ok(())
    }
    ...
    pub async fn handle_cache_write(&self, key: String, _value: Vec<u8>) -> Result<(), CoherenceError> {
    ...
    pub async fn handle_invalidation_message(&self, message: ClusterMessage) -> Result<(), CoherenceError> {
        if let ClusterPayload::CacheOperation { ref key, ref operation, ref data } = message.payload {
            match operation {
                CacheOp::Invalidate => {
                    debug!("Processing cache invalidation for key: {}", key);
                    self.stats.invalidations_received.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    
                    // Apply invalidation locally
                    self.apply_local_invalidation(&key).await?;
                    
                    // Acknowledge invalidation
                    self.acknowledge_invalidation(message.message_id.clone(), message.sender_id.clone()).await?;
                }
                CacheOp::Update => {
                    ...
                    if self.config.conflict_detection {
                        if let Some(conflict) = self.detect_conflict(&key, &message).await? {
                            ...
                            self.stats.conflicts_detected.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                            
                            // Resolve conflict
                            self.resolve_conflict(conflict).await?;
                            self.stats.conflicts_resolved.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        }
                    }
            ...
            self.update_invalidation_log(key.clone(), *operation, message).await;
        }
    ...
    async fn invalidate_cluster_key(&self, key: &str) -> Result<(), CoherenceError> {
        ...
        self.cluster.send_multicast_message(invalidation_message).await
            .map_err(CoherenceError::Network)?;
        
        self.stats.invalidations_sent.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    async fn apply_local_update(&self, key: &str, data: Vec<u8>) -> Result<(), CoherenceError> {
        // This would be implemented by the actual cache
        debug!("Applying local update for key: {} ({} bytes)", key, data.len());
        Ok(())
    }
    ...
    /// Acknowledge invalidation
    async fn acknowledge_invalidation(&self, message_id: String, sender_id: String) -> Result<(), CoherenceError> {
        // Send acknowledgment back to sender
        debug!("Acknowledging invalidation {} from {}", message_id, sender_id);
        Ok(())
    }
    
    /// Detect conflicts for a cache operation
    async fn detect_conflict(&self, key: &str, message: &ClusterMessage) -> Result<Option<CacheConflict>, CoherenceError> {
                ...
                return Ok(Some(CacheConflict {
                    ...
                    conflicting_operations: vec![
                        ...
                        ConflictingOperation {
                            ...
                            operation: if let ClusterPayload::CacheOperation { operation, .. } = &message.payload {
                                operation.clone()
                            } else {
                                CacheOp::Update
                            },
    ...
    async fn resolve_conflict(&self, conflict: CacheConflict) -> Result<(), CoherenceError> {
        match self.config.conflict_resolution {
            ConflictResolution::LastWriterWins => {
                ...
                debug!("Conflict resolved: last writer wins ({})", latest_op.node_id);
                // Apply the latest operation
            }
            ConflictResolution::FirstWriterWins => {
                // Find the operation with the earliest timestamp
                let first_op = conflict.conflicting_operations
                    .iter()
                    .min_by_key(|op| op.timestamp)
                    .unwrap();
                
                debug!("Conflict resolved: first writer wins ({})", first_op.node_id);
    ...
    async fn resolve_with_vector_clock(&self, conflict: &CacheConflict) -> Result<(), CoherenceError> {
    ...
    /// Update invalidation log
    async fn update_invalidation_log(&self, key: String, operation: CacheOp, message: ClusterMessage) {
        ...
        log.entries.retain(|_, entry| entry.timestamp > retention_cutoff);
    }
    
    /// Get coherence statistics
    pub fn stats(&self) -> CoherenceStatsSnapshot {
        CoherenceStatsSnapshot {
            invalidations_sent: self.stats.invalidations_sent.load(std::sync::atomic::Ordering::Relaxed),
            invalidations_received: self.stats.invalidations_received.load(std::sync::atomic::Ordering::Relaxed),
            conflicts_detected: self.stats.conflicts_detected.load(std::sync::atomic::Ordering::Relaxed),
            conflicts_resolved: self.stats.conflicts_resolved.load(std::sync::atomic::Ordering::Relaxed),
            sync_operations: self.stats.sync_operations.load(std::sync::atomic::Ordering::Relaxed),
            failed_invalidations: self.stats.failed_invalidations.load(std::sync::atomic::Ordering::Relaxed),
