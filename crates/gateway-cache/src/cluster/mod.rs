...
impl UDPMulticastCluster {
    ...
    pub async fn new(config: ClusterConfig) -> Result<Self, ClusterError> {
        ...
        if let IpAddr::V4(multicast_ip) = multicast_addr.ip() {
            ...
            return Err(ClusterError::Configuration(
                "Only IPv4 multicast is supported".to_string(),
            ));
        }
    ...
    pub async fn join_cluster(&mut self) -> Result<(), ClusterError> {
        info!(
            ...
            self.node_id, self.config.multicast_address
        );
    ...
    pub async fn send_multicast_message(
        ...
        match self.socket.send_to(&data, multicast_addr) {
            Ok(bytes_sent) => {
                debug!(
                    "Sent {} bytes to multicast group: {:?}",
                    bytes_sent, message.message_type
                );
                self.stats
                    .messages_sent
                ...
                if message.message_type == MessageType::Heartbeat {
                    ...
                    MessageType::CacheInvalidate | MessageType::CacheUpdate
                ) {
                    self.stats
                        .cache_operations_sent
                        .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                }
                ...
                match cluster.socket.recv_from(&mut buffer) {
                    Ok((bytes_received, sender_addr)) => {
                        if let Err(e) = cluster
                            .handle_received_message(&buffer[..bytes_received], sender_addr)
                            .await
                        {
    ...
    async fn handle_received_message(
        ...
        if !self.verify_message_checksum(&message) {
            warn!(
                "Received message with invalid checksum from {}",
                message.sender_id
            );
    ...
    pub fn stats(&self) -> ClusterStatsSnapshot {
        ClusterStatsSnapshot {
            ...
            heartbeats_received: self
                ...
                .heartbeats_received
                .load(std::sync::atomic::Ordering::Relaxed),
            cache_operations_sent: self
                .stats
                .cache_operations_sent
                .load(std::sync::atomic::Ordering::Relaxed),
            cache_operations_received: self
                .stats
                .cache_operations_received
                .load(std::sync::atomic::Ordering::Relaxed),
            message_send_errors: self
                .stats
                .message_send_errors
                .load(std::sync::atomic::Ordering::Relaxed),
            message_receive_errors: self
                .stats
                .message_receive_errors
                .load(std::sync::atomic::Ordering::Relaxed),
        }
    ...
    async fn handle_message(
        ...
        match message.message_type {
            MessageType::Heartbeat => {
                if let ClusterPayload::Heartbeat {
                    load,
                    memory_usage,
                    cache_size,
                } = message.payload
