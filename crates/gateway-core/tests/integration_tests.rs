...
async fn test_gateway_initialization() {
    ...
    assert!(
        ...
        "Gateway should be healthy after initialization"
    );
}
...
async fn test_waf_sql_injection_detection() {
    ...
    use gateway_waf::{RequestContext, WafConfig, WafEngine, WafResult};
...
async fn test_ip_filter() {
    ...
    assert!(filter
        .check_ip(IpAddr::from([192, 168, 1, 100]))
        .await
        .unwrap());
...
async fn test_rate_limiter() {
    use gateway_database::DatabaseManager;
    use gateway_waf::{RateLimitConfig, RateLimiter, RequestContext};
    use std::collections::HashMap;
    use std::net::IpAddr;
