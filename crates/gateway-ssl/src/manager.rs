...
impl SslManager {
    ...
    pub async fn new(config: SslConfig) -> Result<Self> {
        ...
        let storage: Arc<dyn CertificateStorage> = match config.certificate.storage_backend.as_str() {
            ...
            "vault" => {
                ...
                let vault_config = config.vault.as_ref()
                    .ok_or_else(|| SslError::ConfigError("Vault configuration required for vault storage".to_string()))?;
                Arc::new(VaultStorage::new(vault_config.clone())?)
            }
            backend => {
                return Err(SslError::ConfigError(format!("Unsupported storage backend: {}", backend)));
            }
        };
        ...
        let acme_client = if config.auto_ssl.enabled {
            ...
            client.create_account(config.acme.terms_of_service_agreed).await?;
            Some(client)
        } else {
            None
        };
    ...
    pub async fn request_certificate(&self, domain: &str) -> Result<Certificate> {
        ...
        let acme_client = acme_client_guard.as_ref()
            .ok_or_else(|| SslError::AcmeError("ACME client not initialized".to_string()))?;
        
        // Request certificate from ACME
        let cert_pem = acme_client.request_certificate(&[domain.to_string()]).await?;
        ...
        self.store.insert(domain, certificate.clone());
        self.storage.store_certificate(domain, &certificate.info).await?;
        
        info!("Certificate provisioned successfully for domain: {}", domain);
        Ok(certificate)
    }
    ...
    async fn provision_certificates(&self) -> Result<()> {
        ...
        for domain in &self.config.auto_ssl.domains {
            ...
            if let Some(cert) = self.store.get(domain) {
                if !cert.info.needs_renewal(self.config.auto_ssl.renewal_threshold_days) {
            ...
            match self.request_certificate(domain).await {
                Ok(_) => {
                    info!("Successfully provisioned certificate for domain: {}", domain);
                }
                Err(e) => {
                    error!("Failed to provision certificate for domain {}: {}", domain, e);
                }
            }
        }
    ...
    pub fn get_certificate_info(&self, domain: &str) -> Option<crate::certificate::CertificateInfo> {
    ...
    pub fn check_certificate_status(&self, domain: &str) -> Option<CertificateStatus> {
        self.store.get(domain).map(|cert| {
            ...
            } else if cert.info.needs_renewal(self.config.auto_ssl.renewal_threshold_days) {
    ...
    fn create_certificate_from_info(&self, cert_info: crate::certificate::CertificateInfo) -> Result<Certificate> {
        ...
        let chain_pem = if let Some(chain) = &cert_info.certificate_chain {
